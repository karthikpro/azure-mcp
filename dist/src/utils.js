...
const dangerousWiqlPattern = /\b(delete|drop|truncate|insert|update|alter|create|merge|exec(?:ute)?|grant|revoke)\b|;|--|\/\*/i;
...
export const validateWiql = (query) => {
    const trimmed = query.trim();
    if (trimmed.length === 0) {
        throw new McpError('VALIDATION_ERROR', 'WIQL query must not be empty.');
    }
    if (dangerousWiqlPattern.test(trimmed)) {
        throw new McpError('VALIDATION_ERROR', 'WIQL query contains blocked tokens or comment syntax.');
    }
    if (!/\bselect\b/i.test(trimmed) || !/\bfrom\s+workitems\b/i.test(trimmed)) {
        throw new McpError('VALIDATION_ERROR', 'WIQL query must target WorkItems and include a SELECT clause.');
    }
    return trimmed;
};
...
export const errorResult = (error) => {
    ...
    return {
        content: [
            {
                type: 'text',
                text: `ERROR: [${normalized.code}] ${normalized.message}\nDetails: ${JSON.stringify(normalized.details ?? {}, null, 2)}`,
            },
        ],
        isError: true,
    };
...
export const buildOrganizationUrl = (configuredOrgUrl, organization) => {
    if (!organization) {
...
export const parseWorkItemIdFromUrl = (url) => {
...
export const buildWorkItemResourceUri = (organization, project, id) => `azure-devops://${organization}/${project}/workitems/${id}`;
export const parseResourceUri = (uri) => {
...
export const compactObject = (value) => Object.fromEntries(Object.entries(value).filter(([, entryValue]) => entryValue !== undefined));
export const toJsonPatch = (fields, rev) => {
    const operations = [];
